...
`*arData` 之后的==1;;正==向偏移结构体数组，存储真正的 ==1;;Key、Value==，存放 ==1;;8== 个 `Bucket`，占用 ==1;;$8 \times 32 = 256$== 字节。
...
因为[[Hash算法与数据库实现#`union` 的设计]]，`*arHash`和`*arData` 左边的 32 字节是**合法分配**给这个 HashTable 使用的。`*arHash` 指针只是这块连续内存的一个“反向访问入口”。
```c
