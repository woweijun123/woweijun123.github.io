---
{"dg-publish":true,"permalink":"/Learn/Hash算法与数据库实现/","title":"Hash算法与数据库实现","tags":["flashcards"],"noteIcon":"","created":"2026-03-16T11:50:24.000+08:00","updated":"2026-03-16T11:50:24.000+08:00"}
---
...
- **Index Table（逻辑地图）**：位于 `*arData` ==1;;负==向偏移区域（如 -32 字节），负责将 Key 的哈希值映射为具体的数组下标。
- **Bucket 数组（物理仓库）**：位于 `*arData` ==1;;正==向偏移区域（如 +256 字节），按插入顺序存放真实的 Key、Value 及冲突链指针。
#### HashTable「控制器」
```c
...
##### 核心推论：
由于容量（座位）受限于物理仓库 `arData` 的大小，当 `arData` 满载时，即便 `arHash`（指示牌）还有空余槽位，也必须触发扩容和同步 Rehash。
#### C源码 hash 增、改「数字」
```c
