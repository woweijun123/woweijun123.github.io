---
{"dg-publish":true,"permalink":"/work/databases/my-sql/advance/2/","title":"2、索引优化分析","tags":["flashcards"],"noteIcon":"","created":"2026-03-07T22:41:24.000+08:00","updated":"2026-03-10T15:14:33.000+08:00"}
---
...
LIMIT 
    < limit_number >
```
![](http://wcj.x3322.net:88/i/2023/09/20/650ab4d252c19.png)
>fo j w g h sol (佛教温哥华搜了)

mysql七种JOIN理论
![](http://wcj.x3322.net:88/i/2023/09/20/650ab4d38a4b0.png)
# 索引简介
## 索引是什么
**排好序**的快速查找**数据结构**
![](http://wcj.x3322.net:88/i/2023/09/20/650ab4d50230d.png)
...
#### 检索原理
![](http://wcj.x3322.net:88/i/2023/09/20/650ab4d6b6938.png)
##### 初始化介绍
...
![](http://wcj.x3322.net:88/i/2023/09/20/650ab4d82cd8e.png)

![](http://wcj.x3322.net:88/i/2023/09/20/650ab4d9d3259.png)

![](http://wcj.x3322.net:88/i/2023/09/20/650ab4dae5f21.png)
...
![](http://wcj.x3322.net:88/i/2023/09/20/650ab4dbd307d.png)
...
![](http://wcj.x3322.net:88/i/2023/09/20/650ab4744d46a.jpg)
## possible_key介绍
...
![](http://wcj.x3322.net:88/i/2023/09/20/650ab4de26655.png)
## key_len介绍
1. 表示索引中使用的字节数,可通过该列计算查询中使用的索引的长度,在不损失精确性的情况下,长度越短越好
2. key_len显示的值为索引字段的最大可能长度,并非实际使用长度,即key_len是根据表定义计算而得,不是通过表内检索出的
![](http://wcj.x3322.net:88/i/2023/09/20/650ab4df1d2fa.png)

## ref介绍
显示索引的哪一列被使用于查询,如果可能的话,是一个常数。哪些列或常量被用于查找索引列上的值
![](http://wcj.x3322.net:88/i/2023/09/20/650ab4e0217b7.png)
## rows介绍
根据表统计信息及索引选用情况,大致估算出找到所需的记录所需要读取的行数
![](http://wcj.x3322.net:88/i/2023/09/20/650ab4e13c819.png)
## Extra介绍
...
![|1200](http://wcj.x3322.net:88/i/2023/09/20/650ab4e26bad8.png)
...
![](http://wcj.x3322.net:88/i/2023/09/20/650ab4e39882a.png)
...
![](http://wcj.x3322.net:88/i/2023/09/20/650ab4e47a815.png)
...
![](http://wcj.x3322.net:88/i/2023/09/20/650ab4e59e922.png)
### - Start temporary, End temporary
表示半连接中使用了DuplicateWeedout策略的临时表，具体实现过程如下图所示：

![](http://wcj.x3322.net:88/i/2023/09/20/650ab4e672f47.png)
...
![](http://wcj.x3322.net:88/i/2023/09/20/650ab4e80e186.png)
...
![](http://wcj.x3322.net:88/i/2023/09/20/650ab4e92cea1.png)
...
![](http://wcj.x3322.net:88/i/2023/09/20/650ab4ea3b384.png)
...
![](http://wcj.x3322.net:88/i/2023/09/20/650ab4eabf6cd.png)
...
![](http://wcj.x3322.net:88/i/2023/09/20/650ab4ec0b1ff.png)
...
### No tables used
	...
	(没有FROM子句或者使用DUAL虚拟表)
![](http://wcj.x3322.net:88/i/2023/09/20/650ab4ecee4fe.png)
...
![](http://wcj.x3322.net:88/i/2023/09/20/650ab4ee403ca.png)
...
![](http://wcj.x3322.net:88/i/2023/09/20/650ab4ef2d63e.png)
...
# 热身case
![](http://wcj.x3322.net:88/i/2023/09/20/650ab4f0431ae.png)
## 第一行(执行顺序4):   
...
![](http://wcj.x3322.net:88/i/2023/09/20/650ab4f112bf3.png)

2. 最佳左前缀法则(联合索引,从索引的最左前列开始并且不跳过索引中的列)
![](http://wcj.x3322.net:88/i/2023/09/20/650ab4f2b4a8f.png)

3. 不在索引列上做任何操作(计算、函数、(自动or手动)类型转换), 会导致索引失效而转向全表扫描 
![](http://wcj.x3322.net:88/i/2023/09/20/650ab4f39b60c.png)

4. 存储引擎不能使用索引中范围条件右边的列(范围之后全失效)
注:name用于查询，age用于排序
![](http://wcj.x3322.net:88/i/2023/09/20/650ab4f4caf8e.png)

5. 尽量使用覆盖索引,减少select * 会导致索引失效(锅盖理论)
![](http://wcj.x3322.net:88/i/2023/09/20/650ab4f637415.png)
正确使用覆盖索引方式
![](http://wcj.x3322.net:88/i/2023/09/20/650ab4f70ab2b.png)

6. mysql在使用不等于(!=或者<>)的时候无法使用索引会导致全表扫描
![](http://wcj.x3322.net:88/i/2023/09/20/650ab4f825938.png)

7. is null, is not null也无法使用索引
![](http://wcj.x3322.net:88/i/2023/09/20/650ab4f8e6af8.png)

8. like以通配符开头('%ab..)mysql索引失效会变成全表扫描的操作;
![](http://wcj.x3322.net:88/i/2023/09/20/650ab4fa4ae7b.png)
正确使用like索引(LIKE百分写最右，与范围查询的区别在于，左边是定值)
![](http://wcj.x3322.net:88/i/2023/09/20/650ab4fb45370.png)
用覆盖索引解决like '%字符串%'时索引失效的情况
![](http://wcj.x3322.net:88/i/2023/09/20/650ab4fbe0546.png)

9. 字符串不加单引号索引失效(重罪)
![](http://wcj.x3322.net:88/i/2023/09/20/650ab4fd2a95c.png)

10. 少用or,用它来连接时会索引失效
![](http://wcj.x3322.net:88/i/2023/09/20/650ab4fe4e277.png)

# 索引优化总结
## 小总结
![](http://wcj.x3322.net:88/i/2023/09/20/650ab4ff11a68.png)
## 口诀
全值匹配我最爱, 最左前缀要常在
带头大哥不能散, 中间兄弟不能断
索引列上不计算, 范围之后全完蛋
...
| 查询方式               | 性能排名       | 关键原因                 |
| ------------------ | ---------- | -------------------- |
| **JOIN 关联**        | **Top 1**  | 单次请求，充分利用索引，适合报表和列表。 |
...
```
* **为什么快：**
	* **减少往返：** 只需要与数据库进行一次 **网络I/O** 交互「磁盘I/O视情况而定」。
...
### 核心建议
1. **索引是前提：** 无论 SQL 多完美，如果 `items` 表没有建立 `INDEX(shop_id, order_no)`，以上查询都会变慢。
