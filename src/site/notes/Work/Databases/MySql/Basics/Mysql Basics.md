---
{"dg-publish":true,"permalink":"/Work/Databases/MySql/Basics/Mysql Basics/","title":"Mysql Basics","tags":["#flashcards"],"noteIcon":"","created":"2026-09-01T09:56:06.000+08:00","updated":"2026-09-01T09:56:06.000+08:00","dg-note-properties":{"title":"Mysql Basics","tags":["#flashcards"],"reference linking":null}}
---
...
| **$max\_id$** | ==1;;预分配==事务ID<br>`low_limit_id`  | **系统全局变量**：生成 Read View 时，读取系统当前==1;;下一个==待分配事务 ID。                         | **高水位线**：$T_{id} \ge max\_id$ 的版本属于未来==1;;未开启==的事务，可见范围为==1;;否==。 |
### $m\_ids$ 来源与生成机制
...
### 示例：可重复读 (RR)
#### 前提
- 为避免混淆，以下事务分别修改不同的记录（`id=1`～`id=5`）；规则 1 看事务 A 对 `id=1` 的修改，规则 2～5 分别看其他记录的版本；
- 事务 A 在 RR 下第一次普通 `SELECT` 时才生成 Read View，且之后仍**未提交**。
- 纯 `BEGIN; SELECT` 的只读事务没有事务 ID，规则 1 不适用。

事务 ID 在**首次写入**时分配，按本例连续递增：
1. 事务 A 执行 `UPDATE ... WHERE id = 1`，获得 `C_id = 100`，但尚**未提交**。
2. 事务 B 执行 `UPDATE ... WHERE id = 2`，获得 `T_id = 101`，随后提交。
3. 事务 C 执行 `UPDATE ... WHERE id = 3`，获得 `T_id = 102`，保持**未提交**。
4. 事务 D 执行 `UPDATE ... WHERE id = 4`，获得 `T_id = 103`，随后提交。
5. 事务 A 第一次普通 `SELECT`，生成 Read View；
	- 此时下一待分配 ID 为 `104`，**活跃事务只有 B**，因此 `min_id = 102`、`max_id = 104`、`m_ids = {102}`。
6. 事务 A 生成 Read View 后，事务 E 执行 `UPDATE ... WHERE id = 5`，获得 `T_id = 104`。

下面各规则是**独立的版本判定案例**；同一行的并发 `UPDATE` 会受到行锁约束。事务 A 是查询主体，事务 B～E 依次对应规则 2～5 的版本来源。

**Read View (事务 A) 状态**：
* $C_{id} = 100$
* $min\_id = 102$
* $max\_id = 104$
* $m\_ids = \{102\}$（当前活跃未提交的事务 ID 列表）
以下各表均表示**事务 A 对各个 `id` 执行普通 `SELECT` 时**的独立判定；“场景”中的 B、C、D、E 只是被检查版本的创建事务。
#### 如何阅读规则
1. 先看标题中的查询行：本例每个规则分别查询不同的 `id`，不是连续查询同一行。
2. 再看该行当前版本的 `T_id`，并将它与 A 的 `C_id`、`min_id`、`max_id` 和 `m_ids` 比较。
3. 按**规则 1 → 规则 2 → 规则 3 → 规则 4/5** 的顺序判断；命中一条规则后立即结束。
#### 规则 1：当前事务自己的修改（事务 A 查询 `id=1`）
| 场景      | $T_{id}$ | 判定             | 结果     | 原因                                |
| :------ | :------- | :------------- | :----- | :-------------------------------- |
| 事务 A 修改 | 100      | $100 = C_{id}$ | **可见** | 引擎**最先**检查规则 1，即使 $100 < min\_id$ |
#### 规则 2：太旧的已提交版本（事务 A 查询 `id=2`）
| 场景       | $T_{id}$ | 判定          | 结果     | 原因                  |
| :------- | :------- | :---------- | :----- | :------------------ |
| 事务 B 已提交 | 101      | $101 < 102$ | **可见** | 早于 $min\_id$，快照前已结束 |
#### 规则 3：未来的版本（事务 A 查询 `id=5`）
| 场景              | $T_{id}$ | 判定            | 结果      | 原因          |
| :-------------- | :------- | :------------ | :------ | :---------- |
| 事务 E 在 A 生成 Read View 后启动 | 104      | $104 \ge 104$ | **不可见** | **未来**事务的修改 |
#### 规则 4：快照瞬间仍活跃（事务 A 查询 `id=3`；$m\_ids$ 内，RR 反直觉点）
| 场景               | $T_{id}$ | 判定               | 结果      | 原因                                                              |
| :--------------- | :------- | :--------------- | :------ | :-------------------------------------------------------------- |
| 事务 C 拍快照时未提交，后提交 | 102      | $102 \in m\_ids$ | **不可见** | 拍快照时 C 仍**未提交**故进入 $m\_ids$；RR 下名单**冻结**，C 后续提交也不会从 $m\_ids$ 移除 |
#### 规则 5：已提交但不在活跃列表内（事务 A 查询 `id=4`）
| 场景 | $T_{id}$ | 判定 | 结果 | 原因 |
| :--- | :--- | :--- | :--- | :--- |
| 事务 D 快照前已提交 | 103 | $103 \notin m\_ids$ | **可见** | 在 $[min\_id, max\_id)$ 内但快照前已退出活跃链 |
### 快速记忆
1. `T_id = C_id`：事务 A 自己的修改可见。
2. `T_id < min_id`：快照前已提交，可见；`T_id >= max_id`：快照后的版本，不可见。
3. `min_id ≤ T_id < max_id`：在 `m_ids` 中不可见，不在其中可见。
4. RR 冻结 `Read View`；不可见时沿 `Undo` 版本链回溯。RC 则每次 `SELECT` 重建 `Read View`，详见 [[Work/Databases/MySql/Basics/Mysql Basics#MVCC 与事务隔离级别\|#MVCC 与事务隔离级别]]。
<!--SR:!2026-10-28,85,270-->
<?e?>
## MVCC 与事务隔离级别
