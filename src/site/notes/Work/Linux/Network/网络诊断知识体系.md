---
{"dg-publish":true,"permalink":"/Work/Linux/Network/网络诊断知识体系/","title":"网络诊断知识体系","tags":["#review","#network","#diagnose"],"noteIcon":"","created":"2026-07-18T21:37:26.000+08:00","updated":"2026-07-18T21:37:26.000+08:00","dg-note-properties":{"title":"网络诊断知识体系","tags":["#review","#network","#diagnose"]}}
---
...
| `curl`        | HTTP/HTTPS测试 | `-I` <br>`-v`<br>`-w 格式`<br>`-k`<br>`--connect-timeout 5` | `-I`：仅获取**响应头**（快速判断服务是否可达、状态码、证书信息）；<br>`-v`：显示**详细连接过程**（DNS、TCP/TLS握手、请求/响应头，便于排查问题）；<br>`-w`：**自定义输出结果**（如总耗时、HTTP状态码、远端IP等），常用于脚本化监控与性能分析。<br>`-k`：**跳过 TLS** 证书校验（适合自签证书测试环境）；<br>`--connect-timeout 5`：连接阶段**超时设为 5 秒**（仅限制建连时间，不限制整体请求耗时）。 |
