...
<?e?>
# 切片
切片与数组的区别是? <?:?> 切片是数组的包装，提供动态扩容能力。数组无扩容能力
<!--SR:!2026-03-15,39,308-->
<?e?>
1. 使用 make 申明?
2. 字面量声明?
3. 赋值与获取?
...
```
# 指针
申明指针数组的写法是？<?:?> `var ptr [MAX]*int`
<!--SR:!2026-03-16,41,290-->
...
5. 复杂约束调用?
<?l?>
泛型方法的指针地址替换操作`lst.tail.next = &element[T]{val: v}`类似[[Work/Frontend/JS/JS basic#浅拷贝、深拷贝\|JS basic#浅拷贝、深拷贝]]的直接赋值`copy.b = [3]`
```go
package main
...
# 反射
## 基础
通过反射获取结构体方法，必须保持什么一致？否则不能获取结构体的所有方法 <?:?> 接受者类型
<!--SR:!2026-05-12,70,270-->
```go
package main
...
// =============================================================
// 核心理论：反射的三大定律
// 1. 从接口值到反射对象：TypeOf(i) 获取类型, ValueOf(i) 获取值。
// 2. 从反射对象到接口值：v.Interface() 将反射对象重新装箱。
// 3. 修改反射对象：必须是“可寻址”的 (通过指针 + Elem)。
// =============================================================
...
/*
专家提示 (Expert Tips):
1. 性能：反射比直接调用慢 10-100 倍。它是将“编译时检查”推迟到“运行时计算”。
2. 缓存：在高性能 ORM 或 JSON 库中，通常会用 sync.Map 缓存 Type 的 Field 索引和 Tag 解析结果。
3. 谨慎：反射可以读取私有字段(PkgPath != "")，但绝对无法通过反射 SetString 修改它们（除非使用 unsafe）。
4. 类型安全：反射绕过了编译器检查。如果 SetString 到了一个 int 字段，程序会直接 Panic。
5. 对齐：Method.Call 对参数极其严格。传入 Value 还是 Pointer 必须与 Method.Type().In(i) 严格匹配。
*/
func main() {
	// ---------------------------------------------------------
	// 第一部分：元数据探索 (The "Inspect" Phase)
	// 重点：TypeOf 获取的是元数据，关注 Kind (本质类别) 与 Type (定义名称)
	// ---------------------------------------------------------
	u := User{"Gemini", 18}
	t := reflect.TypeOf(u)
	...
	for i := 0; i < t.NumField(); i++ {
		...
		fmt.Printf("4. PkgP: %-10v | 权限判断：为空则可导出，非空则私有\n", f.PkgPath)
		fmt.Printf("5. Offs: %-10v | 内存对齐偏移字节\n", f.Offset)
		fmt.Printf("6. Index: %-10v | 嵌套寻址", f.Index)
		fmt.Printf("7. Anonymous: %-10v | 匿名/嵌入", f.Anonymous)
		fmt.Printf("8. Tag : %-10v | 原始标签\n", f.Tag)
		fmt.Printf("9. JSON: %-10v | 标签解析：Get(\"json\") 结果\n", f.Tag.Get("json"))
	}
...
为了公平，我们将 PHP 的结果换算为纳秒（ns/op）。假设你 PHP 的循环次数是1000000次：

| 项目 | Go (ns/op) | PHP (ns/op) | 性能倍数 (Go vs PHP) |
| --- | --- | --- | --- |
| **原生赋值 (Native)** | **0.26 ns** | ~17.93 ns | **约 68 倍** |
| **反射赋值 (Reflect)** | **1.58 ns** | ~82.98 ns | **约 52 倍** |
**核心发现：**
Go 的**反射调用**（1.58 ns）比 PHP 的**直接赋值**（17.93 ns）还要快 **11 倍**。这意味着在 Go 中即便是被认为“慢”的反射，其执行效率也远超 PHP 的常规代码。
#### 2. 损耗比分析 (Performance Penalty)
我们看看开启反射后，各自比原生慢了多少：
- **Go**: $1.58 / 0.26 \approx$ **6 倍**
- **PHP**: $0.0829 / 0.0179 \approx$ **4.6 倍**
有趣的事实：虽然 Go 的绝对速度快，但从比例上看，Go 开启反射后的性能跌幅（6倍）比 PHP（4.6倍）更大。这是因为 Go 原生代码太快了，导致反射带来的“类型检查”和“内存寻址”开销显得异常突出。
...
##### 1.58 ns/op 的反射快在哪里？
你在 Go 中可能使用了 `v.Field(i).Set()` 而不是 `v.FieldByName("Name").Set()`。
* `1.58 ns` 相当于 **7 个时钟周期**。
...
| ------------------ | -------------- | --------------- | ----------- | --------------------------------- |
| **Native**         | **0.62 ns**    | ~16 亿次          | **1x (基准)** | 最纯粹的内存写入，几乎等同于 CPU 频率的物理极限。       |
| **ReflectByIndex** | **2.22 ns**    | ~4.5 亿次         | **~3.5x**   | 缓存了索引后的反射。这是在生产环境中使用反射的**黄金标准**。  |
| **ReflectByName**  | **25.29 ns**   | ~0.4 亿次         | **~40x**    | 未经优化的反射。每次都靠字符串匹配。这是大部分初级框架的性能瓶颈。 |
##### 为什么内存分配（B/op）都是 0？
...
即便在 Apple M4 这种强大的 CPU 上，**字符串匹配** 依然是沉重的负担。
- **ByIndex (2.22 ns)**：底层只是一次简单的指针偏移计算（Pointer Arithmetic）。CPU 知道起始地址，加上索引偏移，直接定位。
- **ByName (25.29 ns)**：
    1. 拿字符串 "Name" 去结构体的元数据表里对比。
    2. 逐个遍历字段：是 "Age" 吗？不是。是 "Name" 吗？匹配成功。
