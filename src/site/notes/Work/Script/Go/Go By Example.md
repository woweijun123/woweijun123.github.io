...
#### 1. 核心分工
- **`TypeOf`（元数据）**：
	- 负责描述“类型结构”，如字段名、Tag、类型名称。它**不持有任何实例数据**。
	- 关注 **Kind**（本质类别，如 `struct`）与 **Type**（定义名称，如 `User`）。
- **`ValueOf`（实体值）**：
	- 负责操作“实例数据”。它**包含 `Type` 的所有信息**，并能进行存取与修改。
	- 关注 **Data**（运行时具体数值）与 **Settability**（是否可修改）。
#### 2. 总结对比
| 特性 | `reflect.TypeOf(x)` | `reflect.ValueOf(x)` |
| :--- | :--- | :--- |
...
##### 凭空创造构造体
```go
// 1. 【字段建模】手动构建结构体字段序列。每个 StructField 定义了名称、类型及元数据 (Tag)
...
// 2. 【类型合成】将字段序列转换为运行时唯一的结构体类型描述符
// 此时在内存中“凭空”生成了一个全新的类型元数据
tDynamic := reflect.StructOf(fields)

// 3. 【堆内存分配】按合成类型的尺寸开辟零值内存，返回指向该空间的指针 Value
// 语义等同于：ptr := new(DynamicStruct)
vPtr := reflect.New(tDynamic)

// 4. 【寻址与填充】通过 .Elem() 穿透指针，利用字段索引进行内存偏移寻址并写入数据
// 提示：动态生成的结构体字段通常建议按索引（Field）访问以保证性能
vPtr.Elem().Field(0).SetString("GhostBot")
vPtr.Elem().Field(1).SetInt(1024)
...
type OrderServiceInterface interface {
	PlaceOrder(orderInfo *Order) *StatusDo
	CancelOrder(orderInfo *Order) *StatusDo
}
...
// 下单
func (s *OrderService) PlaceOrder(req *Req) *StatusDo {
	fmt.Printf("【下单】订单ID: %s\n", req.OrderInfo.Id)
	return &StatusDo{Status: 1}
}

// 取消
func (s *OrderService) CancelOrder(req *Req) *StatusDo {
	fmt.Printf("【取消】原因: %s, 订单ID: %d\n", (*req.Extend)["reason"], req.OrderInfo.Id)
	return &StatusDo{Status: 2}
}

// 请求参数结构体
type Req struct {
	OrderStatus *StatusDo `json:"order_status"`
...
func call(orderAction OrderAction, req *Req) any {
	// 获取方法缓存
	method, ok := EventCache.Load(orderAction)
	if !ok {
...
func main() {
	...
	for i := 0; i < vo.NumMethod(); i++ {
		method := vo.Method(i)
		methodType := to.Method(i)
		EventCache.Store(methodType.Name, method)
	}
