...
> ```
<!--SR:!2026-08-19,43,290-->
<?e?>
### 6、不能使用短变量声明这种方式来设置字段值
#### 💡注意事项
- **左侧纯名原则**：`:=` 左侧只能出现==1;;变量==名，不能出现 ==1;;`obj.Field`、`arr[i]`、`*ptr`==。
- **作用域陷阱**：如果在内部作用域（如 `if` 块）再次使用 `err :=`，会产生一个==1;;隐藏外部变量的新 `err`==，这经常导致**逻辑错误**。
- **混合声明**：`x, err := work()` 只要 ==1;;`x`== 是新定义的，==1;;`err`== 可以是旧的，但这仅限于 `err` 是纯==1;;变量==名的情况。
#### 赋值符号对比表
| **语法**              | **操作性质** | **左侧要求**        | **适用场景**      |
| ------------------- | -------- | --------------- | ------------- |
| **`var x T = ...`** | 显式声明     | 变量名             | 标准变量声明        |
| **`x := ...`**      | 简短声明     | 必须全部是==1;;变量名== | 函数内部快速定义新变量   |
...
> }
> ```
<!--SR:!2026-07-31,25,270-->
<?e?>
### 14、Slice和Array是一维的
Go 并**不直接支持原生的多维数组**，其本质是==1;;数组的数组==或==1;;切片的切片==。对于数值计算，开发者需权衡性能与灵活性。
#### 两种动态构建方式对比
| **方式**                 | **构建步骤**                     | **特点**               | **适用场景**            |
| ---------------------- | ---------------------------- | -------------------- | ------------------- |
| **独立切片 (Independent)** | 1. 创建外层切片<br>2. 循环为每行分配内存    | 每行长度可变，内存地址不连续。      | ==1;;每一行长度不固定的稀疏矩阵。==      |
| **共享内存 (Shared Data)** | 1. 创建一维大连续内存<br>2. 划分给外层切片引用 | 内存连续，CPU 缓存命中率高，性能好。 | ==1;;高性能数值计算、固定尺寸矩阵。== |
> [!question]- 示例代码
> ```go
> h, w := 2, 4
...
> }
> ```
<!--SR:!2026-08-13,38,290-->
<?e?>
### 16、字符串的不可变性
在 Go 语言中，字符串（String）是==1;;只读的字节序列==。这意味着一旦创建，你==1;;无法==通过下标操作直接修改字符串中的某个字符。
> [!question]- 示例代码
> ```go
> x := "text"
> // x[0] = 'T' // ❌error
...
> ok      benchmark       4.238s
> ```
<!--SR:!2026-08-20,43,250-->
<?e?>
### 18、字符串和索引操作符
在 Go 语言中，字符串的索引操作返回的是**原始字节（==1;;byte/uint8==）**，而非直观上的**字符**。
...
> ```
<!--SR:!2026-08-13,48,215-->
<?e?>
### 19、字符串并不总是UTF8的文本
在 Go 语言中，字符串本质上是==1;;只读的字节切片（ReadOnly Byte Slice）==，并不==1;;强制==要求符合 ==1;;UTF-8== 编码。
...
3. **协议控制符/魔数**：
	* 在==1;;网络==协议或==1;;二进制==文件中，常被用作消息的==1;;开始/结束==边界符或特定数据的特征码。
...
> ```
<!--SR:!2026-08-15,39,290-->
<?e?>
### 21、在多行切片、数组和映射文字中缺少逗号
在 Go 语言中，当**复合字面量**（Slice、Array、Map、Struct）==1;;分多行书写==时，每一行（包括最后一行）都**必须**以 ==1;;逗号 `,`== 结尾；==1;;单行书写==时末尾逗号可选。
> [!question]- 示例代码
> ```go
...
> z := []int{1, 2,}
> ```
<!--SR:!2026-08-22,45,290-->
<?e?>
### 22、log.Fatal 与 log.Panic 的终止效应
在 Go 中，标准库 `log` 的 `Fatal` 和 `Panic` 函数会**直接干预程序的运行生命周期**：
- ==1;;`log.Panic`==：打印后触发 panic，==1;;可被 recover 捕获==
- ==1;;`log.Fatal`==：打印后调用 ==1;;`os.Exit(1)`==，==1;;不会执行任何 defer==
> [!question]- 示例代码
> ```go
...
> ```
<!--SR:!2026-08-16,41,250-->
<?e?>
### 23、内置数据结构非线程安全
虽然 Go 原生支持高并发（Goroutines），但其内置的数据结构（如 **Maps**、**Slices**）==1;;并不是==并发安全的。多个 Goroutine 同时读写同一个变量会导致 ==1;;Data Race==（数据竞态），并发写 Map 会直接 ==1;;panic: concurrent map writes==。正确做法：==1;;`sync.Mutex`== 加锁，或读多写少时用 ==1;;`sync.Map`==。
> [!question]- 示例代码
> ```go
...
<?e?>
### 27、自增与自减的语法约束
Go 语言对 `++` 和 `--` 运算符进行了严格限制：它们被定义为==1;;语句（Statement）而非表达式（Expression）==。这意味着它们==1;;不能==放在等号右侧，也==1;;不能==作为函数参数，且==1;;仅支持后置==。
> [!question]- 示例代码
> ```go
> i := 1
>
...
<?e?>
### 29、位运算优先级与主流语言的显著差异
**位运算**（与、或、异或、取反）==1;;优先级高于四则运算==（加、减、乘、除、取余），==1;;有别于 C 语言==。
> [!question]- 示例代码
> ```go
> // 1. 位与 (&) vs 加法 (+)
...
<!--SR:!2026-08-02,48,254-->
<?e?>
### 32、无缓冲通道的发送行为与主协程退出竞争
无缓冲通道（Unbuffered Channel）是==1;;同步==的。**发送者**在消息被**接收者接收的那一刻**就会**解除阻塞**，但这==1;;并不意味着==接收者已经完成了对该消息的处理。
> [!question]- 示例代码
> ```go
> ch := make(chan string)
> go func() {
...
方法接收者（Receiver）本质上是函数的第一个参数。如果使用**值接收者**，方法内部得到的是原始数据的一份==1;;完整拷贝==；如果使用**指针接收者**，则得到指向原数据的指针，==1;;可修改原结构体字段==。值接收者仍可通过==1;;指针字段==或==1;;map== 修改引用内容。
> [!question]- 示例代码
> ```go
> package main
>
...
#### 1. 使用 `==` 操作符的硬性条件
你可以对结构体变量使用 `==`，前提是其内部每一个字段都是==1;;可比较==的。

| **大分类**   | **子分类** | **具体类型**                                               | **可比较性**  | **比较规则与注意事项**                                   |
| --------- | ------- | ------------------------------------------------------ | --------- | ----------------------------------------------- |
| **可比较**   | 基础值类型   | `bool`, `int`, `float`, `string`, `complex`, `uintptr` | ✅         | 比较其实际存储的数值或文本内容。                                |
|           | 引用标识    | 指针 (Pointer)                                           | ✅         | 比较==1;;内存地址==。指向同一变量则相等。                        |
|           |         | 通道 (Channel)                                           | ✅         | 比较==1;;引用==。指向同一个 `make` 创建的实例则相等。              |
| **不可比**   | 动态容器    | 切片 (Slice)                                             | ❌         | 仅能与 ==1;;`nil`== 比较。原因：深度比较开销大且语义模糊。            |
|           | 哈希容器    | 映射 (Map)                                               | ❌         | 仅能与 ==1;;`nil`== 比较。原因：内部顺序不确定。                 |
|           | 逻辑实体    | 函数 (Function)                                          | ❌         | 仅能与 ==1;;`nil`== 比较。无法定义逻辑上的“相等”。               |
| **条件可比较** | 集合类型    | 数组 (Array)                                             | **取决于元素** | 只有当元素类型本身可比较时，数组才可比较。                           |
|           | 复合类型    | 结构体 (Struct)                                           | **取决于字段** | 只有当所有成员字段都可比较时，结构体才可比较。                         |
|           | 抽象类型    | 接口 (Interface)                                         | **运行时决定** | 编译期允许比较，但若动态值为不可比类型（如 Slice），运行时会 ==1;;Panic==。 |
##### 1.1 可比较的结构体示例
...
* **风险**：这些函数在遇到第一个不匹配的字节时会立即返回，攻击者可以通过观察响应时间来逐位破解（==1;;计时攻击==）。
* **方案**：使用 ==1;;`crypto/subtle.ConstantTimeCompare()`==。
<!--SR:!2026-07-28,19,210-->
<?e?>
