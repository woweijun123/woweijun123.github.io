---
{"dg-publish":true,"permalink":"/Work/Script/PHP/Basics/Encrypt/","title":"Encrypt","tags":["flashcards"],"noteIcon":"","created":"2026-09-03T09:55:24.000+08:00","updated":"2026-09-03T09:55:24.000+08:00","dg-note-properties":{"title":"Encrypt","tags":["flashcards"],"reference linking":null}}
---

# 常见加密算法速查
## 对称加密
| 名称/组合                 | 作用                              | 加密强度              | 性能          | 优势                  | 劣势                               | 典型使用场景                    |
| :-------------------- | :------------------------------ | :---------------- | :---------- | :------------------ | :------------------------------- | :------------------------ |
| **AES-256-GCM**       | 使用同一把密钥加密和解密数据，并验证完整性（**AEAD**） | **高（256-bit 密钥）** | **极快，硬件加速** | 安全性成熟、标准化程度高、硬件支持广  | 通信双方需要安全分发和保管密钥；**Nonce 绝不能复用**  | **HTTPS/TLS**、数据库字段、文件加密  |
| **ChaCha20-Poly1305** | 加密数据并验证完整性（**AEAD**）            | **高（256-bit 密钥）** | **极快，软件友好** | 不依赖 AES 硬件；实现高效     | 生态和硬件加速普及度不如 AES；**Nonce 绝不能复用** | **TLS**、**SSH**、WireGuard |
| **AES-128-GCM**       | 使用同一把密钥加密和解密数据，并验证完整性（**AEAD**） | **高（128-bit 密钥）** | **极快，硬件加速** | 密文短、兼容性好、资源消耗较低     | 密钥强度低于 AES-256；Nonce 绝不能复用       | **HTTPS/TLS**、高吞吐服务、数据库字段 |
| **SM4**               | 国密对称加密                          | **高（128-bit 密钥）** | **快**       | 符合**中国商用密码标准**，性能较好 | 国际生态和跨平台兼容性不如 AES                | 国密 TLS、政企系统、需要国密合规的存储和传输  |
## 非对称加密
| 名称/组合 | 作用 | 加密强度 | 性能 | 优势 | 劣势 | 典型使用场景 |
| :-- | :-- | :-- | :-- | :-- | :-- | :-- |
| **ECC（椭圆曲线密码学）** | 公钥密码学体系，具体算法包括 ECDH、ECDSA 等 | **高（256-bit 曲线约 128-bit 安全强度）** | **快，密钥短** | 相同安全强度下密钥更短、计算和传输开销更小 | 不是单一算法；曲线和实现选择较复杂，兼容性需注意 | **HTTPS/TLS** 密钥交换和证书签名、**SSH** 密钥、物联网设备 |
| **ECDH/ECDHE（P-256、X25519）** | 协商共享会话密钥 | **高（约 128-bit 安全强度）** | **快，低开销** | ECDHE 可提供**前向保密** | 本身不是数据加密算法；实现和曲线选择更复杂 | **HTTPS/TLS** 密钥交换，配合 AES-GCM 或 ChaCha20-Poly1305 加密实际数据 |
| **ECDSA P-256 / Ed25519** | 数字签名与验签 | **高（约 128-bit 安全强度）** | **快，签名短** | 性能好；Ed25519 使用简单且安全性高 | 只提供签名，不提供数据保密；兼容性取决于协议和库 | TLS 证书签名、**SSH 密钥**、软件发布签名 |
| **SM2（256-bit）** | 国密公钥加密、密钥交换和数字签名 | **高（约 128-bit 安全强度）** | **快** | 符合中国商用密码标准 | 国际生态和跨平台兼容性有限 | 国密证书、国密 TLS、政企系统 |
| **RSA-3072** | 公钥加密小数据、私钥解密；或数字签名 | **高（约 128-bit 安全强度）** | **慢，开销大** | 生态成熟、证书和工具支持广 | 不适合直接加密大量数据；计算和传输开销较大 | TLS 证书签名、软件/邮件签名、密钥封装 |
| **RSA-2048** | 公钥加密小数据、私钥解密；或数字签名 | **中高（约 112-bit 安全强度）** | **慢，开销大** | 兼容性最好、部署广泛 | 安全强度低于 RSA-3072；不适合直接加密大量数据 | TLS 证书签名、软件/邮件签名、遗留系统 |

**协议与格式对应关系：** **HTTPS（TLS 1.3）** 通常使用 ECDHE 密钥交换、AES-GCM 或 ChaCha20-Poly1305 加密数据，再用 RSA/ECDSA/Ed25519 证书签名认证。**JWT** 默认只是 Base64URL 编码；HS256 使用 HMAC-SHA-256，RS256 使用 RSA，ES256 使用 ECDSA，EdDSA 使用 Ed25519，只有 **JWE** 才提供加密。**SSH** 常使用 Curve25519/ECDH 密钥交换、AES-CTR/GCM 或 ChaCha20-Poly1305 加密，以及 Ed25519/RSA 签名认证。

**说明：** 表中的加密强度是常见配置下的近似安全等级，实际强度还取决于密钥长度、曲线、工作模式、Nonce/IV 管理和实现质量。大量数据优先使用 **AES-GCM** 或 **ChaCha20-Poly1305**；密码存储应使用 `password_hash`（bcrypt/Argon2 等慢哈希），不要使用可逆加密或 MD5；Base64 和 URL 编码都不是加密。

# 非对称加密
非对称加密使用一对公钥和私钥，可用于**密钥交换、数字签名**，以及对少量数据进行可逆加密；它与不可逆的**单向散列**不是同一种技术。
## RSA
**RSA** 是一种**非对称加密算法**，它利用**大整数分解的数学难题**来确保安全。它使用一对密钥：**公钥**和**私钥**。
- **公钥加密**：任何人都可以使用你的**公钥**来**加密**数据。
- **私钥解密**：只有拥有相应**私钥**的人才能**解密**数据。
在实践中，由于 RSA 算法的**计算量大，效率较低**，它**很少**用于直接**加密大量数据**。
它通常用于 **“密钥封装” (key wrapping)**：也就是用**公钥**加密一个**对称密钥**，然后用这个**对称密钥去加密真正的数据**。
### RSA 的限制
- **加密数据长度限制**：
...
```
#### RSA 加密与解密
**历史上的 TLS**：RSA 可用于加密并交换对称加密的会话密钥，但 RSA 不适合直接加密大量业务数据。
访问 `https://example.com` 时，现代 TLS 1.3 通常使用 ECDHE 协商会话密钥，再用 AES-GCM 或 ChaCha20-Poly1305 加密实际数据；服务器证书中的 RSA/ECDSA/Ed25519 密钥主要用于身份认证和签名。
```php
// 加密数据（使用公钥加密）
$data      = "Hello, RSA!";
$encrypted = '';
