---
{"dg-publish":true,"permalink":"/Work/Script/PHP/Frame/Hyperf/Hyperf 协程解析/","title":"Hyperf 协程解析","tags":["flashcards"],"noteIcon":"","created":"2025-09-12T09:40:38.522+08:00","updated":"2026-03-24T17:32:02.026+08:00","dg-note-properties":{"title":"Hyperf 协程解析","tags":["flashcards"],"reference linking":null}}
---
...
下面是协程调度器工作的流程图，它清晰地展示了协程在不同状态间的转换和调度过程：
...
1.  **Channel（通道）**：类似于 Go 语言的 `chan`，为**多生产者协程和多消费者协程模式**提供支持。Channel 底层自动实现了协程的切换和调度。
    *   `push()`：生产数据。如果通道已满，则协程挂起。
    *   `pop()`：消费数据。如果通道为空，则协程挂起，直到有数据到来。

2.  **WaitGroup**：基于 Channel 实现，用于**等待一组协程全部执行完毕**。
    ```php
...
3.  **Parallel**：这是对 WaitGroup 的一个更高级的封装，让你能更方便地**并行执行多个任务并收集结果**。你还可以**限制最大并发数**，防止瞬时并发过高压垮下游服务。
    ```php
...
4.  **Concurrent**：用于**控制一个代码块内同时运行的最大协程数量**。
    ```php
...
#### 解决方案：
**显式复制**：在创建子协程时，手动将父协程的上下文复制过去。
```php
$parentId = Co::id(); // 获取父协程ID
co(function () use ($parentId) {
	Context::copy($parentId); // 关键：复制父协程上下文
	// ... 现在可以获取到父协程的上下文数据了
});
```
**链式查找**：通过 `Coroutine::pid()` 获取父协程 ID，然后逐级向上查找需要的上下文数据。